  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <StatCard
          label="PENDING"
          value={data.all.pending}
          icon={Clock}
          color="text-amber-400"
          sub={`${pct(data.all.pending, data.all.total)}% unactioned`}
        />
        <StatCard
          label="QUALIFIED"
          value={data.all.qualified}
          icon={CheckCircle2}
          color="text-emerald-400"
          sub={`${pct(data.all.qualified, data.all.total)}% advance`}
        />
        <StatCard
          label="ELIMINATED"
          value={data.all.eliminated}
          icon={XCircle}
          color="text-red-400"
          sub={`${pct(data.all.eliminated, data.all.total)}% out`}
        />
      </div>
        ...
        {[
          ...
          <div key={tLabel} className="bg-[#0A0A0A] border border-white/[0.06] rounded-lg p-4">
            ...
            <div className="space-y-2">
              {[
                { l: 'Qualified', v: stats.qualified, c: 'bg-emerald-500' },
                { l: 'Eliminated', v: stats.eliminated, c: 'bg-red-500' },
                { l: 'Pending', v: stats.pending, c: 'bg-amber-500/50' },
              ].map(({ l, v, c }) => (
                <div key={l} className="flex items-center gap-2">
                  ...
                  <div className="flex-1 h-1.5 bg-white/[0.03] rounded-full overflow-hidden">
                    <div
                      className={`h-full ${c} rounded-full transition-all duration-500`}
                      style={{ width: `${pct(v, stats.total)}%` }}
                    />
                  </div>
                  <span className="text-[9px] font-mono font-bold text-gray-400 w-6 text-right">
                    {v}
                  </span>
                </div>
              ))}
...
function AttributionTable({ round }: { round: '1' | '2' }) {
  ...
  const filtered = useMemo(() => {
    ...
    return teams.filter(
      (t) => !q || t.name.toLowerCase().includes(q) || t.shortCode.toLowerCase().includes(q)
    );
  }, [teams, search]);
          ...
          <tbody>
            {filtered.map((t, i) => {
              ...
              return (
                <tr
                  key={t.id}
                  className={`border-b border-white/[0.03] ${i % 2 === 0 ? 'bg-transparent' : 'bg-white/[0.01]'} hover:bg-white/[0.03]`}
                >
                  ...
                  <td className="py-2 px-3">
                    <span
                      className={t.track === 'IDEA_SPRINT' ? 'text-cyan-400' : 'text-orange-400'}
                    >
                  ...
                  <td className="py-2 px-3 text-gray-600">
                    {actionAt
                      ? new Date(actionAt).toLocaleString('en-IN', {
                          dateStyle: 'short',
                          timeStyle: 'short',
                        })
                      : '—'}
...
export function EliminationAnalytics() {
  ...
  const totalProgress =
    data.round1.all.total > 0
      ? Math.round(
          ((data.round1.all.qualified + data.round1.all.eliminated) / data.round1.all.total) * 100
        )
      : 0;
    ...
    <div className="space-y-8">
      ...
      <div className="bg-[#0A0A0A] border border-white/[0.06] rounded-xl p-4">
        ...
        <div className="h-3 bg-white/[0.04] rounded-full overflow-hidden flex mb-2">
          <div
            ...
            style={{
              width: `${data.round1.all.total > 0 ? (data.round1.all.qualified / data.round1.all.total) * 100 : 0}%`,
            }}
          />
          <div
            className="h-full bg-red-500/80 transition-all duration-700"
            style={{
              width: `${data.round1.all.total > 0 ? (data.round1.all.eliminated / data.round1.all.total) * 100 : 0}%`,
            }}
          />
          <div
            className="h-full bg-amber-500/40 transition-all duration-700"
            style={{
              width: `${data.round1.all.total > 0 ? (data.round1.all.pending / data.round1.all.total) * 100 : 0}%`,
            }}
          />
        </div>
        <div className="flex items-center gap-5">
          <span className="text-[9px] font-mono text-emerald-500 flex items-center gap-1">
            ...
            Qualified: {data.round1.all.qualified}
          </span>
          <span className="text-[9px] font-mono text-red-500 flex items-center gap-1">
            ...
            Eliminated: {data.round1.all.eliminated}
          </span>
          <span className="text-[9px] font-mono text-amber-500 flex items-center gap-1">
            ...
            Pending: {data.round1.all.pending}
          </span>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2">
          <h3 className="text-[10px] font-mono font-bold text-gray-400 tracking-widest">
            TEAM-WISE JUDGE ATTRIBUTION
          </h3>
          <div className="flex items-center gap-1 ml-auto">
            {(['1', '2'] as const).map((r) => (
              <button
                key={r}
                onClick={() => setAttrRound(r)}
