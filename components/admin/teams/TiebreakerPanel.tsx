    ...
    <div className="fixed right-0 top-0 h-full w-full max-w-md bg-[#0A0A0A] border-l border-white/[0.08] z-50 flex flex-col shadow-2xl">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {isLoading ? (
          ...
          trackData.tieGroups.map(
            (
              group: {
                score: number;
                teams: { id: string; name: string; score: number; manualRank: number | null }[];
                resolved: boolean;
              ...
              >
                ...
                <div className="divide-y divide-white/[0.03]">
                  {group.teams.map(
                    (
                      team: { id: string; name: string; score: number; manualRank: number | null },
                      ti: number
                    ) => {
                              ...
                              <>
                                <button
                                  onClick={() =>
                                    setEditingRanks((prev) => ({
                                      ...prev,
                                      [team.id]: String(team.manualRank ?? ''),
                                    }))
                                  }
              ...
              {Object.entries(RESOLUTION_LABELS)
                ...
                .map(([key, label]) => (
                  <div key={key} className="flex items-center gap-2">
                    <span
                      className={`text-[8px] font-mono px-1.5 py-0.5 rounded border ${RESOLUTION_COLORS[key]}`}
                    >
                      {key === 'top_criterion'
                        ? 'TOP‑CRIT'
                        : key === 'judge_count'
